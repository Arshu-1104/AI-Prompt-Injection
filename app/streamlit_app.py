...
def get_analyzer(model_name: str) -> "PromptAnalyzer | None":
    if model_name not in st.session_state["analyzers"]:
        try:
            ...
            st.error(
                f"Could not load the '{model_name}' model: {exc}\n\n"
    ...
    if st.button("Analyze Prompt", type="primary"):
        if not user_text.strip():
            ...
            if selected_model == "bert":
                ...
                st.caption("Token highlights are only available with the BERT model.")
